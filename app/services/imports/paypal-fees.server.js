...
export async function importPaypalPayoutCsv({ storeId, csv }) {
  if (!storeId) {
    throw new Error("storeId is required for PayPal import");
  }
  if (!csv) {
  ...
  if (!rows.length) {
    throw new Error("未检测到有效的 PayPal 记录");
  }
...
function parseCsv(csv) {
  ...
  const idIdx = headers.findIndex((h) => ["payout id", "transaction id", "id"].includes(h));
